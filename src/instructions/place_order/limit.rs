...
pub fn execute_limit_order(accounts: &mut [AccountView], args: &PlaceOrderArgs) -> ProgramResult {
    ...
    // Maker accounts, looked up by pubkey match rather than raw index — see
    // find_maker_account's doc comment in mod.rs for why.
    let remaining_accounts = &mut accounts[6..];
    ...
    unsafe {
        ...
        // 1. Same-book crossing: mirrors market.rs's bid/ask crossing exactly (same fee
        // split, same balance updates), just bounded by the limit price instead of
        // walking the whole book. This is what was previously missing entirely from the
        // limit-order path — a resting Sell and an incoming Buy on the same outcome
        // (or vice versa) would never cross, only ever rest.
        let target_seats_ptr = view_target.seats.as_mut_ptr();

        if args.side == 0 {
            // Buy taker: matches resting Sells on the target book, cheapest first.
            for price in 1..=(args.price as usize) {
                if taker_remaining == 0 {
                    break;
                }
                let level = &mut view_target.directory[100 + price];

                while taker_remaining > 0 && level.head != 0 {
                    let head_idx = level.head as usize;
                    let maker_order = &mut view_target.orders[head_idx];
                    ...
                    let trade_collateral = (match_qty * price as u64) / 100;
                    let fee = calculate_symmetric_fee(match_qty, price as u8, fee_rate_bps);
                    ...
                    if maker_order.quantity == 0 {
                        ...
                        maker_order.next_idx = view_target.header.next_free_node_idx;
                        view_target.header.next_free_node_idx = level.head;
                        level.head = next_head;
                        if next_head == 0 {
                            level.tail = 0;
                        }
                    }
                    if maker_seat.collateral_locked == 0 && maker_seat.ot_a_locked == 0 && maker_seat.ot_b_locked == 0 {
                        maker_seat.market_user_state = Address::default();
                    }
                }
            }
        } else {
            // Sell taker: matches resting Buys on the target book, richest first.
            for price in (args.price as usize..=99).rev() {
                if taker_remaining == 0 {
                    break;
                }
                let level = &mut view_target.directory[price];
        ...
        // 2. Cross-Orderbook Matching (combo mint, Buy side only): a resting Buy on the
        // complementary outcome at a price that sums to >= 100 with the taker's price
        // synthesizes both outcome legs from combined collateral.
            ...
            for comp_price in (1..=99).rev() {
                ...
                while taker_remaining > 0 && comp_level.head != 0 {
                    ...
                    if maker_order.quantity == 0 {
                        ...
                        // The combo mechanic never locks collateral/OT on the maker's seat
                        // (it mints from the taker's collateral directly), so a maker's
                        // seat here only ever needs evicting if it also has no other
                        // resting liabilities left over from the same-book crossing above
                        // or a separate resting order.
                        if maker_seat.collateral_locked == 0 && maker_seat.ot_a_locked == 0 && maker_seat.ot_b_locked == 0 {
                            maker_seat.market_user_state = Address::default();
                        }
                    }
